...
from botCommands.musicbot import linkutils, utils
...
class Music(commands.Cog):
    ...
    async def _shuffle(self, ctx):
        ...
        if current_guild is None:
            embed = discord.Embed(title=config.NO_VOICE_CHANNEL,
    ...
    async def _queue(self, ctx):
        ...
        currSong = utils.guild_to_audiocontroller[current_guild].current_song

        # Embeds are limited to 25 fields
        if config.MAX_SONG_PRELOAD > 25:
            config.MAX_SONG_PRELOAD = 25

        embed = discord.Embed(title=f":scroll: Queue [{len(playlist.playque)}]",
                              description=f"current:\n**{currSong.info.title}**", color=config.EMBED_COLOR)

        for counter, song in enumerate(list(playlist.playque)[:config.MAX_SONG_PRELOAD], start=1):
            if song.info.title is None:
                embed.add_field(name=f"{song.info.webpage_url}",
                                value=f"{str(counter)}. {song.info.webpage_url}", inline=False)
            else:
                embed.add_field(name=f"{song.info.title}",
                                value=f"{str(counter)}. {song.info.webpage_url}", inline=False)
    ...
    @commands.command(
        ...
        aliases=["np", " np", " songinfo", "info", " info"])
