...
from botCommands.musicbot import utils
...
class General(commands.Cog):
    ...
    async def _connect(self, ctx):  # dest_channel_name: str
        embed = discord.Embed(title="Connected!.", colour=config.EMBED_COLOR)
    ...
    async def _disconnect(self, ctx, guild=False):
        embed = discord.Embed(title="Disconnected!", colour=config.EMBED_COLOR)
        await ctx.send(embed=embed)
        current_guild = utils.get_guild(self.bot, ctx.message)
        audiocontroller = utils.guild_to_audiocontroller[current_guild]
